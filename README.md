...
- Le décodage d'une image (ou la détection d'objets/visages) peut lui aussi rester bloqué indéfiniment sur certains fichiers, sans lever d'erreur — ce qui arrêtait auparavant le scan net après la toute première photo traitée, sans jamais passer à la suivante. Cette étape est désormais bornée par un délai de sécurité (45 s) : au-delà, la photo concernée est marquée en échec (retentée au scan suivant) et le traitement continue avec les photos restantes.
