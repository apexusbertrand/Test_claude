...
- Certains dossiers protégés par le système (ex. le dossier de l'appareil photo sur Android) peuvent refuser la création du sous-dossier `photos/miniatures` par défaut, même avec la permission d'écriture accordée. Si l'analyse remonte une erreur de permission, utilisez le bouton optionnel *"Utiliser un autre emplacement pour les miniatures"* pour les stocker ailleurs (ex. la racine du stockage).
