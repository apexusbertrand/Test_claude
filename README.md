...
- Certains dossiers protégés par le système (ex. le dossier de l'appareil photo sur Android) peuvent refuser l'écriture, même avec la permission accordée. Sur certains navigateurs Android, ce refus ne déclenche ni erreur ni message : l'opération reste bloquée indéfiniment. Toute opération d'écriture sur le disque est donc bornée par un délai de sécurité (20 s) qui la transforme en erreur explicite si elle ne répond pas — utilisez alors le bouton optionnel *"Utiliser un autre emplacement pour les miniatures"* pour les stocker ailleurs (ex. la racine du stockage).
- **L'IA ne peut jamais bloquer les miniatures.** La miniature de chaque photo est générée et écrite sur le disque *avant* toute analyse IA. Les modèles IA sont la partie la plus fragile sur mobile : ceux de détection d'objets sont téléchargés depuis un CDN externe (`storage.googleapis.com`) et ceux de visages passent par WebGL — l'un comme l'autre peuvent rester bloqués indéfiniment sans jamais échouer, ce qui figeait auparavant le scan juste après la première miniature. Chaque étape IA est donc bornée (25 s pour charger un modèle, 15 s par photo).
- **Un échec sur *une* photo n'éteint plus l'IA pour toutes les autres.** Le chargement du modèle (au tout début du scan) et son exécution sur chaque photo sont deux choses distinctes : seul un échec de **chargement** (modèle injoignable/inutilisable) désactive la fonctionnalité pour le reste de la session — un échec ponctuel de détection sur une seule photo (photo inhabituelle, timeout isolé) ne fait plus que sauter le tag IA de cette photo-là, les suivantes sont retentées normalement. C'est notamment nécessaire pour la détection d'objets/animaux : `@tensorflow-models/coco-ssd` (dernière version publiée, 2.2.3, le paquet n'est plus maintenu) a un bug connu où sa table de 90 catégories COCO comporte 10 identifiants réservés/inutilisés — une prédiction, même faible, tombant sur l'un d'eux fait planter l'appel avec `Cannot read properties of undefined (reading 'displayName')` sur une photo par ailleurs tout à fait normale.
- **Vérification unique en amont, pas à chaque photo.** Les deux modèles (objets/animaux et visages) sont chargés **en parallèle** avant de traiter la moindre photo, pas paresseusement à la première — un modèle qui échoue coûte au plus son propre délai (25 s), une seule fois pour tout le scan, jamais 25-45 s par photo. Si un module ne peut pas être chargé, une **alerte explicite** apparaît (et reste consultable dans le panneau de diagnostic) précisant lequel et pourquoi le tag correspondant n'a pas pu être généré.
- Le décodage d'une image peut lui aussi rester bloqué sur certains fichiers ; cette étape est bornée à 45 s, au-delà desquelles la photo est marquée en échec (retentée au scan suivant) et le traitement continue avec les suivantes.
- **Journal visible à l'écran** : un téléphone n'ayant pas de console de développement, tout incident (modèle IA indisponible, photo non décodable, scan interrompu) s'affiche dans un bandeau de diagnostic directement dans la page, refermable d'un clic.
...
- **Lecture d'une miniature déjà générée.** Sur certains Android, un handle de fichier restauré depuis IndexedDB (après un rechargement de page, ou juste après le scan qui vient de l'écrire) peut occasionnellement échouer ou rester bloqué à la lecture même si le fichier existe bel et bien sur le disque. Cette lecture est donc bornée (8 s) et, en cas d'échec, un nouveau handle est immédiatement redemandé au dossier de l'événement avant d'abandonner ; si les deux tentatives échouent, la carte affiche "⚠ Miniature indisponible" et la raison exacte est journalisée dans le panneau de diagnostic pour permettre un diagnostic depuis le téléphone lui-même.
