...
- **Premier lancement** : deux boutons apparaissent l'un après l'autre — d'abord *"Choisir le dossier de photos"* (le dossier source, jamais modifié), puis *"Choisir l'emplacement des miniatures"* (un dossier séparé — sur Android, choisissez par exemple la racine du stockage interne). Une fois les deux choisis (ou reconnectés via le bouton "🔄"), les miniatures déjà taguées s'affichent aussitôt — **aucune analyse n'est lancée automatiquement**. Cliquez sur *"Analyser le dossier"* quand vous voulez chercher les nouvelles photos, générer leurs tags et miniatures, et réparer celles qui auraient échoué.
