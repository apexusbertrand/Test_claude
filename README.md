...
- **L'IA ne peut jamais bloquer les miniatures.** La miniature de chaque photo est générée et écrite sur le disque *avant* toute analyse IA. Les modèles IA sont la partie la plus fragile sur mobile : ceux de détection d'objets sont téléchargés depuis un CDN externe (`storage.googleapis.com`) et ceux de visages passent par WebGL — l'un comme l'autre peuvent rester bloqués indéfiniment sans jamais échouer, ce qui figeait auparavant le scan juste après la première miniature. Chaque étape IA est donc bornée (25 s pour charger un modèle, 15 s par photo) et, en cas d'échec, la fonctionnalité concernée est **désactivée pour toute la session** au lieu de re-bloquer sur chaque photo : les miniatures, les tags de date/lieu/événement et la recherche continuent normalement.
