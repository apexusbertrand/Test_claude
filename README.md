...
- **Affichage immédiat depuis les miniatures.** Au chargement, l'app essaie de restaurer uniquement l'emplacement des miniatures (indépendamment du dossier source) et affiche aussitôt tout ce qu'il contient — tags modifiables, partage de la miniature — même si le dossier source n'est pas (encore) accessible cette session. Dès que celui-ci redevient accessible (bouton 1, ou restauration automatique de la permission), l'app rattache en tâche de fond le fichier original à chaque photo déjà affichée (pour l'ouverture/le partage de l'original) et recherche les nouvelles photos.
